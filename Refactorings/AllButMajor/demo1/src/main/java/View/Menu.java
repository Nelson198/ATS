...
import Exceptions.InvalidNewRegisterThrowable;
...
public class Menu{
    ...
    public AutonomyCar autonomyCarRent(String error) throws InvalidNewRentalThrowable {
        Scanner scanner = new Scanner(System.in);
        this.displayMenuHeader(error);
    ...
    public CheapestNearCar walkingDistanceRent(String error) throws InvalidNewRentalThrowable {
        Scanner scanner = new Scanner(System.in);
        this.displayMenuHeader(error);
        out.println("Tipo do Carro: [electric, gas, hybrid or any]");
        String carType = scanner.nextLine();
    ...
    public SpecificCar specificCarRent(String error) throws InvalidNewRentalThrowable {
        Scanner scanner = new Scanner(System.in);
        this.displayMenuHeader(error);
    ...
    public RentCarSimple simpleCarRent(String error) throws InvalidNewRentalThrowable {
        Scanner scanner = new Scanner(System.in);
        this.displayMenuHeader(error);
        out.println("Tipo do Carro: [electric, gas, hybrid or any]");
        String carType = scanner.nextLine();
    ...
    public RegisterCar newRegisterCar(String error) throws InvalidNewRegisterThrowable {
        this.displayMenuHeader(error);
        Scanner scanner = new Scanner(System.in);
        out.println("Matricula:");
        ...
        try {
            ...
            throw new InvalidNewRegisterThrowable();
        }
    }

    public RegisterUser newRegisterUser(String error) throws InvalidNewRegisterThrowable {
        ...
        if (this.mMenu.equals(MenuInd.RegisterClient)) {
            try {
    ...
    public Menu parser() {
        ...
        if (str.matches("^[+-]?\\d{1,8}$")) {
            ...
            if (this.options.size() > i - 1 && i > 0) {
                this.prev.push(this.mMenu);
                this.mMenu = this.options.get(i - 1);
                this.pickChildMenus();
            }
        }
        switch (str) {
    ...
    public Menu back() {
        if (this.prev.size() > 0) {
            this.mMenu = this.prev.pop();
            this.pickChildMenus();
        } else {
            this.run = false;
        }
        if (this.mMenu.equals(MenuInd.Login) || this.mMenu.equals(MenuInd.Register))
            this.back();
        return this;
    }
    ...
    private String createHeader() {
        ...
        return strHeader.append(this.mMenu.name()).append("--\n").red().toString();
    }
